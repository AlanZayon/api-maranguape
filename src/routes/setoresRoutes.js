...
router.get("/dados/:setorId", async (req, res) => {
  try {
    ...
    const coordenadoriasComFuncionarios = await Promise.all(
      coordenadorias.map(async (coordenadoria) => {
        const funcionariosDaCoordenadoria = funcionarios.filter(
          (funcionario) =>
            String(funcionario.coordenadoria) === String(coordenadoria._id)
        );

        const funcionariosComUrls = await Promise.all(
          funcionariosDaCoordenadoria.map(async (funcionario) => {
        ...
        return {
          ...
          funcionarios:
            funcionariosComUrls.length > 0 ? funcionariosComUrls : [],
        };
      })
    );
